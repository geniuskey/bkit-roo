...
| T-064 | api-client 테스트 | P0 | ⬜ | T-052~T-063 | 프로바이더별 단위 테스트, 모의 응답 (SDK 의존성으로 별도) |
...
| T-101 | stdio 트랜스포트 구현 | P1 | ✅ | T-100 | 자식 프로세스 기반 MCP 서버 연결 (stub) |
| T-102 | SSE 트랜스포트 구현 | P2 | ✅ | T-100 | Server-Sent Events 기반 연결 (stub) |
| T-103 | Streamable HTTP 트랜스포트 | P2 | ✅ | T-100 | HTTP 스트리밍 기반 연결 (stub) |
| T-104 | McpHub 구현 | P1 | ✅ | T-101~T-103 | 멀티 서버 연결 허브 |
| T-105 | 도구 발견 및 호출 | P1 | ✅ | T-104 | MCP 서버의 도구 목록 조회 및 실행 (인터페이스) |
| T-106 | 리소스 접근 | P2 | ✅ | T-104 | MCP 서버의 리소스 조회 (인터페이스) |
| T-107 | 설정 파일 로더 | P1 | ✅ | T-100 | mcp_settings.json, .roo/mcp.json 로딩 |
| T-108 | 연결 상태 모니터링 | P2 | ✅ | T-104 | 서버 연결 상태 추적 (getServerStates) |
...
| T-112 | 컨텍스트 윈도우 트래커 | P1 | ✅ | T-111 | 실시간 토큰 사용량 추적 (calculateBudget) |
...
| T-121 | BkitRooConfig 설계 | P0 | ✅ | T-016, T-120 | 전체 설정 통합 인터페이스 (AgentRunnerConfig) |
| T-122 | AgentRunner 구현 | P0 | ✅ | 모든 Phase 3~4 | 에이전트 실행 루프 (메시지 → LLM → 파싱 → 도구 실행 → 반복) |
| T-123 | TaskManager 구현 | P0 | ✅ | T-122 | 태스크 상태 머신 (AgentRunner.runTask 내 통합) |
| T-124 | 스트리밍 이벤트 시스템 | P0 | ✅ | T-122 | `AgentEvent` 기반 실시간 이벤트 스트리밍 |
| T-125 | 설정 프록시(ConfigProxy) | P1 | ⬜ | T-121 | 3계층 저장소 (글로벌, 프로젝트, 시크릿) |
| T-126 | 안전 장치 구현 | P1 | ✅ | T-122 | 연속 실패 감지, 루프 방지 (maxRequests), abort() |
...
| T-141 | VS Code 독립성 검증 | P0 | ✅ | Phase 1~5 | `vscode` import가 없는지 자동 검사 스크립트 (check-vscode) |
...
T-001 (모노레포 초기화) ✅
  └─→ T-010~T-017 (shared 패키지) ✅
        ...
        ├─→ T-070~T-075 (tools) ✅
        ├─→ T-020~T-023 (cost) ✅
        └─→ T-050~T-063 (api-client) ✅
              └─→ T-080~T-084 (modes) ✅
                    └─→ T-090~T-097 (prompts) ✅
                          └─→ T-100~T-108 (mcp-client) ✅
                          └─→ T-110~T-115 (context) ✅
                                └─→ T-120~T-126 (core) ✅
                                      └─→ T-130~T-134 (예제/문서) ✅ (3/5)
                                            └─→ T-140~T-145 (품질/릴리스) 🔄 (1/6)
```

**진행률**: 89/101 태스크 완료 (88%)

**남은 주요 작업**:
- 테스트 추가: T-064 (api-client, SDK 의존성), T-109 (mcp-client), T-127 (core 통합) (3개)
- 문서: T-132 (TSDoc), T-133 (마이그레이션 가이드) (2개)
- 품질/릴리스: T-140, T-142~T-145 (5개)
- 기타 미구현: T-057, T-058, T-059, T-061, T-114, T-125 (6개, P2~P3)
...
| Phase 0: 초기화 | 5 | 5 | 100% |
| Phase 1: 기반 | 9 | 9 | 100% |
| Phase 2: 유틸리티 | 14 | 14 | 100% |
| Phase 3: 핵심 기능 | 37 | 33 | 89% |
| Phase 4: 고급 기능 | 17 | 15 | 88% |
| Phase 5: 통합 | 8 | 6 | 75% |
| Phase 6: 문서 | 5 | 3 | 60% |
| Phase 7: 릴리스 | 6 | 1 | 17% |
| **합계** | **101** | **86** | **85%** |
